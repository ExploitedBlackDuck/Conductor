...
// The change kinds a dry-run preview distinguishes. A delete is the dangerous
// one and is always enumerated; creates/updates may be summarised by count for
// very large trees (§7.4).
const (
...
// ChangeSet is the parsed result of a --dry-run pass: the concrete set of
// creates, updates, and deletes an operation would perform, the basis of the
...
//
// The slices may be capped for very large trees — Truncated then reports that
// CreateCount/UpdateCount exceed the listed entries — but Deletes are never
// capped, so the dangerous changes are always fully enumerable.
type ChangeSet struct {
	Creates []FileChange
	Updates []FileChange
	Deletes []FileChange
	// Exact totals, accurate even when the slices above are truncated.
	CreateCount int
	UpdateCount int
	DeleteCount int
	// Truncated reports that the Creates/Updates slices list fewer entries than
	// their counts (Deletes are always complete).
	Truncated bool
}
