...
**Decision.** Before a destructive operation (sync-with-deletion, `delete`, `purge`, bisync `--resync`) can be confirmed, Conductor runs the *same* operation with `--dry-run` (via the rc `_config.DryRun` override, §7.2.1), parses the result into a concrete change set (creates / updates / **deletes**), and presents it. The confirm control is enabled only after the operator has been shown that change set; the deletes are counted and visually distinct. The change set, and the operator's acknowledgement of it, are hash-chained into the audit log. This is structural, not advisory — the gate is enforced in the core, not surfaced as a dismissible warning.
**How the change set is obtained (not log-scraping).** rclone's human-readable dry-run lines are not a stable interface. Conductor captures the dry-run with structured logging (`_config.UseJSONLog` / equivalent) and classifies each emitted object event (copy / update / **delete-skipped**) into the `ChangeSet`; where a kind exposes a structured dry-run result over rc it is preferred. The parser is table-tested against captured fixtures (§2.10) and is part of the catalog/binary drift guard (§7.5) so a rclone upgrade that changes the dry-run event shape fails CI rather than silently mis-parsing — the verified behaviour is pinned to the rclone version.
...
- **`_config`** — per-call overrides for governance/behaviour (`Transfers`, `Checkers`, `TpsLimit`, `BwLimit`, `DryRun`, `--delete-*` policy, etc.). This is how `Intensity` and the **dry-run preview pass** (ADR-0015) are applied to one call without mutating daemon globals.
...
- **The confirm is gated behind a parsed dry-run preview (ADR-0015).** Before the confirm control is enabled, Conductor runs the same operation with `_config.DryRun` (§7.2.1), parses the output into a `ChangeSet` (creates / updates / **deletes**), and shows it — deletes counted and visually distinct. The operator confirms against *that concrete change set*, not an abstract warning. The change set and its acknowledgement are hash-chained into the audit log (§7.8). For huge trees the preview is virtualized and may summarise by count, but the deletes are always enumerable.
